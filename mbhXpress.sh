...
do
	...
	t) THREADS=${OPTARG};;
	h) echo "mbhXpress v1.2.0 - usage :";
	   echo "-a PROTEOME1";
	   echo "-b PROTEOME2";
	   echo "-o OUTPUT_NAME";
	   echo "-t THREADS";
	   exit;;
	?) echo "mbhXpress v1.2.0 - usage :";
	   echo "-a PROTEOME1";
	   echo "-b PROTEOM2";
	   echo "-o OUTPUT_NAME";
	   echo "-t THREADS";
...
echo "             mbhXpress v1.2.0" | tee -a LOG.$OUTPUT_NAME
echo "---------------------------------------------" | tee -a LOG.$OUTPUT_NAME
...
$diamond_path blastp -q $PROTEOME2 -d "$run_id.p1" -o "$run_id.p2_p1" -k 1 -e 1E-10 --threads $THREADS &> LOG.$OUTPUT_NAME
echo " - run the blast : b vs a" | tee -a LOG.$OUTPUT_NAME
$diamond_path blastp -q $PROTEOME1 -d "$run_id.p2" -o "$run_id.p1_p2" -k 1 -e 1E-10 --threads $THREADS &> LOG.$OUTPUT_NAME
...
echo "   Log written in LOG.$OUTPUT_NAME" | tee -a LOG.$OUTPUT_NAME
echo "---------------------------------------------" | tee -a LOG.$OUTPUT_NAME
######################### DONE
